...
serve-inline provider='{"name":"cursor","command":"agent","args":["acp"]}':
    uv run acp-to-api serve --provider '{{ provider }}'
...
test-langchain:
    uv run pytest tests/test_e2e_langchain_agent.py -v
...
daemon-start:
    uv run acp-to-api start --config acp-to-api.toml

# Stop the daemon
daemon-stop:
    uv run acp-to-api stop

# Show daemon status
daemon-status:
    uv run acp-to-api status

# Restart the daemon
daemon-restart:
    uv run acp-to-api restart --config acp-to-api.toml

# Install as system service (auto-start on boot)
setup:
    uv run acp-to-api setup --config acp-to-api.toml

# Remove system service
setup-uninstall:
    uv run acp-to-api setup --uninstall

# -- Testing ------------------------------------------------------------------

# Run daemon E2E tests
test-daemon:
    uv run pytest tests/test_e2e_daemon.py -v
